...
describe Import::Screenings do
  ...
  describe '#perform_for(cinema)' do
    ...
    let(:screening_1) { instance_double('OdeonUk::Performance') }
    let(:screening_2) { instance_double('OdeonUk::Performance') }
    let(:screening_3) { instance_double('OdeonUk::Performance') }
    let(:screening_4) { instance_double('OdeonUk::Performance') }

    before do
      Timecop.freeze

      expect(OdeonUk::Performance).to receive(:at).with(odeon_brand_id.to_s).and_return([screening_1, screening_2, screening_3, screening_4])

      expect(screening_1).to receive(:dimension).and_return('2d')
      expect(screening_1).to receive(:film_name).and_return('Iron Man 3')
      expect(screening_1).to receive(:starting_at).and_return(1.hour.from_now.utc)
      ...
      expect(screening_2).to receive(:starting_at).and_return(2.hours.from_now.utc)
      ...
      expect(screening_3).to receive(:starting_at).and_return(3.hours.from_now.utc)
      ...
      expect(screening_4).to receive(:starting_at).and_return(4.hours.from_now.utc)
  ...
  describe '#perform(cinema)', 'CineworldUk' do
    ...
    let(:screening_1) { instance_double('CineworldUk::Performance') }
    let(:screening_2) { instance_double('CineworldUk::Performance') }
    let(:screening_3) { instance_double('CineworldUk::Performance') }
    let(:screening_4) { instance_double('CineworldUk::Performance') }

    before do
      Timecop.freeze

      expect(CineworldUk::Performance).to receive(:at).with(cineworld_brand_id.to_s).and_return([screening_1, screening_2, screening_3, screening_4])
